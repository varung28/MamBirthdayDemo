...
PushConstant pushConstant_model_uniformData[3];
...
void uninitialize(void)
{
	...
	if (DRSJ_vkDevice)
	{
		...
		// Destory uniform buffer for zebra_crossing
		if (uniformData_zebra_crossing.vkBuffer)
		{
			vkDestroyBuffer(DRSJ_vkDevice, uniformData_zebra_crossing.vkBuffer, NULL);
			uniformData_zebra_crossing.vkBuffer = VK_NULL_HANDLE;
			fprintf(DRSJ_gpLogFile, "%s %d : uniformData_zebra_crossing.vkBuffer Free\n", __func__, __LINE__);
		}

		if (uniformData_zebra_crossing.vkDeviceMemory)
		{
			vkFreeMemory(DRSJ_vkDevice, uniformData_zebra_crossing.vkDeviceMemory, NULL);
			uniformData_zebra_crossing.vkDeviceMemory = VK_NULL_HANDLE;
			fprintf(DRSJ_gpLogFile, "%s %d : uniformData_zebra_crossing.vkDeviceMemory Free\n", __func__, __LINE__);
		}

		// Destory uniform buffer for rectangle
		if (uniformData_main_road.vkBuffer)
		{
			vkDestroyBuffer(DRSJ_vkDevice, uniformData_main_road.vkBuffer, NULL);
			uniformData_main_road.vkBuffer = VK_NULL_HANDLE;
			fprintf(DRSJ_gpLogFile, "%s %d : uniformData_main_road.vkBuffer Free\n", __func__, __LINE__);
		}

		if (uniformData_main_road.vkDeviceMemory)
		{
			vkFreeMemory(DRSJ_vkDevice, uniformData_main_road.vkDeviceMemory, NULL);
			uniformData_main_road.vkDeviceMemory = VK_NULL_HANDLE;
			fprintf(DRSJ_gpLogFile, "%s %d : uniformData_main_road.vkDeviceMemory Free\n", __func__, __LINE__);
		}
...
VkResult createVertexBuffer(void)
{
	...
	return createVertexBuffer_by_darshan();
}
...
VkResult updateUniformBuffer(void)
{
	...
	return updateUniformBuffer_by_darshan();
}
