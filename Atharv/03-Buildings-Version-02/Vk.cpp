...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(
        WS_EX_APPWINDOW,
        szAppName,
        TEXT("Vulkan 2D Buildings For Gokhale Madam's Birthday"),
...
VkResult createVertexBuffer(void)
{
    ...
    vkResult = createVertexBuffer_buildings();
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s() => createVertexBuffer_buildings() Failed : %d !!!\n", __func__, vkResult);
        return vkResult;
    }    
    else
        fprintf(gpFile, "%s() => createVertexBuffer_buildings() Succeeded\n", __func__);
...
VkResult buildCommandBuffers(void)
{
    ...
    for (uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        vkCmdBeginRenderPass(vkCommandBuffer_array[i], &vkRenderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
        {
            ...
            buildCommandBuffers_buildings(i);
        }
        //* Step - 7
